...
void gainProcessing(double pIn[][BLOCK_SIZE], double pOut[][BLOCK_SIZE])
{
	...
	{
		pIn[LEFT_CH][i] = pIn[LEFT_CH][i] * inputGain;
		pIn[RIGHT_CH][i] = pIn[RIGHT_CH][i] * inputGain;

		pOut[LEFT_CH][i] = saturation(pIn[LEFT_CH][i]);
		pOut[RIGHT_CH][i] = saturation(pIn[RIGHT_CH][i]);

		pOut[LEFTS_CH][i] = pIn[LEFT_CH][i];
		pOut[RIGHTS_CH][i] = pIn[RIGHT_CH][i];

	}
	//MODE0 invertuje 
	if (mode == MODE0)
	{
		gst_audio_invert_transform(&inverter, pIn[LEFT_CH], pOut[LEFT_CH], BLOCK_SIZE);
		gst_audio_invert_transform(&inverter, pIn[RIGHT_CH], pOut[RIGHT_CH], BLOCK_SIZE);
	}
